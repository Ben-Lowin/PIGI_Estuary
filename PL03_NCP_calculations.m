...
load PL_03_deployment_PIGI_RAW_v20250106.mat
%load the imported temp/salintity data
load("PL03_master.mat")
%load historical wind speed data
load("Weather_data_PL03.mat")
%load water heigh
load("NOAA_dec_tide.mat")
...
PL_water_depth = tide.depth +1.2;
% plot(water_height.time,PL_water_depth)
% Looks good
time_with_fix = tide.time + hours(2);

% Now perform the interpolation with unique sample points
mld = interp1(time_with_fix, PL_water_depth, pigi_dat.datetime)';
...
averagedWindTimetable.ws(isnan(averagedWindTimetable.ws)) = 0;
...
for idx = 1:N
    if ~isnat(pigi_dat.datetime(idx))
        ...
        u10mat_new(idx,M) =pigi_wind(idx);
            ...
            %go as far back as there is data and replace nan with 0
            base = zeros(120,1);
            base(abs(d-121):end-1) = averagedWindTimetable.ws(1:d-1);
            %set nan to zero
            base(isnan(base))=0;
            u10mat_new(idx,1:M-1) = base;
            % adds the new data into the last row
            u10mat_new(idx,M) =pigi_wind(idx);
        end
    else
...
clear idx averagedWindTimetable N M pigi_wind ws date_w i d windTimetable base
