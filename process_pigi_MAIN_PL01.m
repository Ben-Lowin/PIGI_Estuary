...
% Directory where data are saved
    data_dir = 'C:\Users\BenL\Documents\MATLAB\PIGI\June_dock_NCP\PIGI_deplymnet_data';
        
% Directory where output/processed data will be saved
    save_dir = 'C:\Users\BenL\Documents\MATLAB\PIGI\June_dock_NCP\PIGI_out';
    
% Directory where output/processing data will be saved
    fig_dir = 'C:\Users\BenL\Documents\MATLAB\PIGI\June_dock_NCP\PIGI_out';

% (Useful) Deployment information
    %REQUIRED:
    dep_info.name = 'PL_01_deployment'; %NOTE - do not use dashes in the name
    dep_info.year = 2024;
    dep_info.start = datestr(datenum(2024,06,16)); %datestring format (e.g. = datestr(datenum(2020,06,01));)
    dep_info.finish = datestr(datenum(2024,06,22)); %datestring format
    %OPTIONAL
    dep_info.id = 'PL01';
